...
class DisbursedCard extends StatefulWidget {
  const DisbursedCard({
    Key? key,
  ...
  @override
  State<DisbursedCard> createState() {
    return _DisbursedCardState();
  }
}

class _DisbursedCardState extends State<DisbursedCard> {
  double rating = 3;
  var ratingText = '';
      ...
      child: Column(
        ...
        children: [
          ...
          setRowColumValueOpenCard(
              "Invoice Date", invoiceDate ?? '-', "Lender", AppUtils.getBankFullName(bankName: bankName ?? '')),
          setRowColumValueOpenCard(
              "Tenure", '$tenure ${tenure == '0' ? 'Day' : 'Days'}', "ROI", '$interestRate% p.a.' ?? '-'),
