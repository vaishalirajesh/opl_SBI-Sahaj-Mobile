          ...
          child: Center(
            child: TextFormField(
              ...
              decoration: InputDecoration(
                prefixIcon: GestureDetector(child: Icon(Icons.arrow_back,color: Colors.white,size: 25.h,),onTap: (){
                  Navigator.pop(context);
                },),
                border: InputBorder.none,
                hintText: 'Search',
                hintStyle: ThemeHelper.getInstance()?.textTheme.button?.copyWith(fontFamily: MyFont.Nunito_Sans_Regular),
              ),
              onChanged: (searchValue){
                setState(() {
                  searchText = searchValue;
                  filterInvoiceList = widget.arrInvoiceList?.where((invoiceData) => invoiceData.buyerName?.toLowerCase().contains(searchText.toString().toLowerCase()) == true || invoiceData.invoiceData?.invNum.toString().toLowerCase().contains(searchText.toString().toLowerCase()) == true || invoiceData.invoiceData?.invValue.toString().toLowerCase().contains(searchText.toString().toLowerCase()) == true || invoiceData.invoiceData?.invDate.toString().toLowerCase().contains(searchText.toString().toLowerCase()) == true).toList(); ;
      ...
      return ListView.builder(
        ...
        itemBuilder: (context, index) {
          return Column(
            children: [
              Container(
                key: searchTextController.text.isEmpty ?  ValueKey(widget.arrInvoiceList?[index].buyerName.toString()) : ValueKey(filterInvoiceList?[index].buyerName.toString()),
                child: searchTextController.text.isEmpty  ? invoiceDataUI(widget.arrInvoiceList?[index]) : invoiceDataUI(filterInvoiceList?[index]),
              ),
              SizedBox(
                height: 5.h,
              ),
      ...
      child: Column(children: [
        Row(
          ...
          children: [
            ...
            Text(
                Utils.convertIndianCurrency(gstInvoiceDataObj?.invoiceData?.invValue?.toString()),
                style: ThemeHelper.getInstance()!
                    .textTheme
                    .bodyText1!
                    .copyWith(
                    fontSize: 16, color: MyColors.pnbcolorPrimary))
          ],
        ),
        SizedBox(
          height: 5.h,
        ...
        Row(
          children: [
            ...
            Text(
                " \u2022 ${gstInvoiceDataObj?.invoiceData?.invNum}" ??
                    "",
                style: ThemeHelper.getInstance()!.textTheme.bodyText2),
          ],
        ),
